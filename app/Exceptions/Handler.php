...
class Handler extends ExceptionHandler
{
    ...
    public function render($request, Throwable $exception)
    {
        ...
        if ($exception instanceof ValidationException) {
            # If it's an API route or expects JSON, return JSON response
            if ($request->is('api/*') || $request->expectsJson()) {
                return $this->ValidationError($exception);
            }

            # Otherwise, fallback to normal web redirect with errors
            return redirect()
                ->back()
    ...
    protected function unauthenticated($request, AuthenticationException $exception)
    {
        ...
        // Determine the guard (e.g., admin, user)
        ...
        return redirect()->guest($login); // Redirects to appropriate login page
    }
