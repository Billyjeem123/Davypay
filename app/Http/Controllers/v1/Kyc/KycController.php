...
class KycController extends Controller
{
    ...
    public function verifyBvn(GlobalRequest $request): JsonResponse
    {
        ...
        try {
            ...
            $webhookData = $this->updateWebhookData($user, 'bvn', $entity);
    ...
    private function updateWebhookData($user, string $type, array $entity): string
    {
        unset($entity[trim($type) === 'driver_license' ? 'photo' : 'image']);
        $webhookRaw = optional($user->kyc)->webhook;
        $existingWebhook = json_decode($webhookRaw, true);
        # If decoding failed or returned anything other than array, fallback to empty array
        if (!is_array($existingWebhook)) {
            $existingWebhook = [];
        }
        $existingWebhook[$type] = $entity;
        return json_encode($existingWebhook);
    }
    ...
    private function saveBase64Image(string $base64Image, string $filename, string $type): ?string
    {
        try {
            ...
            $kycImagesPath = storage_path('app/public/images');
    ...
    public function verifyNin(GlobalRequest $request): JsonResponse
    {
        ...
        try {
            ...
            #  Create KYC record
            $webhookData = $this->updateWebhookData($user, 'nin', $entity);
            $this->createOrUpdateKycRecord($user, [
                'nin' => $nin,
                'phone_number' => $entity['phone_number'] ?? null,
    ...
    public function verifyDriverLicense(GlobalRequest $request): JsonResponse
    {
        ...
        try {
            ...
            $webhookData = $this->updateWebhookData($user, 'driver_license', $entity);
    ...
    public function subscribeToDojah(GlobalRequest $request): JsonResponse
    {
        $validated = $request->validated();

        $url = config('services.dojah.base_url') . '/api/v1/webhook/subscribe';
